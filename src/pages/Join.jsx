...
import{_getUsersName} from "../redux/modules/Login"
...
const Join = () => {
    ...
    const onEmailHandler = (event) => {
        setUeser({...user, email: event.currentTarget.value})
    }
    const onPasswordHandler = (event) => {
        setUeser({...user, password: event.currentTarget.value})
    }

    const onAgeHandler = (event) => {
        setUeser({...user, age: event.currentTarget.value})
    }
    const onSubmitHandler = async(event) => {
        event.preventDefault()
      
        const { data } = await axios.post(
            "http://13.125.24.153/api/member/signup",
            user
          );
      ...
      <div>
        ...
        <form className="input-area" onSubmit={onSubmitHandler}>
          <div className="email-input-area">
            <div>이메일</div>
            <input type="email" id="login"onChange={onEmailHandler} />
            
          </div>
          <div className="pw-input-area">
            <div>비밀번호</div>
            <input type="password" id="password"  onChange={onPasswordHandler}/>
          </div>
          <div className="pw-input-area">
            <div>나이</div>
            <input type="number" id="number"  onChange={onAgeHandler}/>
          </div> 
          
          <div>
            <Btn className="Login-btn" text={"가입하기"} type={"red"} />
          </div>
        </form>
      </div>
    </div>
